...
// class responsible for connection logic. MainActivity also holds some via Listeners.
class ExecutionManager (
    initLatencies : HashMap<String, Int>,
    val receiveMessageListener : (String?) -> Unit,
    val passIdListener : (String?) -> Unit,
    val connectionInfoListener : (String) -> Unit
) {
    ...
    fun connectForLatency(adress: Pair<String, Int>) {
        thread {
            val l : (String?)->Unit = { _: String? ->  Log.i("lass", "unused")}
    ...
    private fun connectToAddress(candidate : String, inetaddress : Pair<String, Int>) {
            try {
                ...
                connected = candidate
                connectionInfoListener("Connected to: ".plus(connected!!).plus(" latency: ").plus(latencies[candidate]))
            } catch (e: Exception) {
        ...
        if(latencyConnection != null && latencyConnection?.connected()!!) {
            thread {
                try {
                    ...
                    js.put("node", s)
                    js.put("latency", l)
                    connectionInfoListener("Connected to: ".plus(connected!!).plus(" latency: ").plus(latencies[connected!!]))
